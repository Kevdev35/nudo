...
export function updateKanbanCardUseCase(kbCard: KanbanCardRepository, kbList: KanbanListRepository, kbBoard: KanbanBoardRepository, projectRepo: ProjectRepository) {
    return async (userId: string, kanbanCardId: string, data: { title?: string; description?: string | null; order?: number; completed?: number }) => {
        ...
        const updatedCard = await kbCard.update(kanbanCardId, data);
