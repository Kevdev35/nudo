...
export function deleteKanbanListUseCase(kbList: KanbanListRepository, kbBoard: KanbanBoardRepository, projectRepo: ProjectRepository) {
    return async (userId: string, kanbanListId: string) => {
        ...
        const deletedList = await kbList.softDelete(kanbanListId);
